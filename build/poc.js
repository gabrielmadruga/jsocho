...
function printt(pattern, txt, x, y, colors, a) {
    x -= a * 4 * txt.length;
...
function randomizer_next(exclude) {
    if (this.forced) {
...
function randomizer_force(values) {
    this.forced = values;
}
// TODO:
...
function e_become(e, new_state) {
    e.state = new_state;
    e.t = 0;
}
function e_is_a(e, tag) {
    return e.tags.indexOf(tag);
}
const g_entities = [];
const g_entities_with = {};
...
//   self:select()
//  end
function pointer_choose(os, interaction, cancellable = false) {
    this.options = os;
...
function runner_unblock(value = null) {
    this.value = value;
    this.blocked = false;
}
function runner_delay(dur) {
    function delay() {
        const delay = {
            ...e_new(),
...
function runner_choose(os, interaction, cancellable) {
    this.blocked = true;
