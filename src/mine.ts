...
function randomField(pred: (f: Field) => boolean) {
  let x, y;
  do {
    x = flr(rnd(board_w));
    y = flr(rnd(board_h));
...
function startGame(mineCount: number) {
  game = {
    state: "playing",
    flags: 0,
    minesFound: 0,
...
function makeFirstVisit(board: Board) {
...
function visit(b: Board, x: number, y: number) {
  const f = safeGet(x, y);
  if (!f || f.visited || f.flagged) return null;
